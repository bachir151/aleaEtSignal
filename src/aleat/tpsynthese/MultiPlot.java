...
import aleat.ReseauRoutier.SemiCircu;
import aleat.ReseauRoutier.SemiCircu2;
import aleat.tpH.*;
import signal.*;
...
//	}
		...
		public void paintComponent(Graphics g1) {
			...
			for (PlotablePlus pp : sortedPlotablePlus) {
				...
				for (int i = 0; i < uneFoncPix[0].length; i++) {
					...
					/** on met des couleurs variantes, si le {@link MultiPlot.MultiPlot.Plotable} est un {@link MultiPlot.MultiPlot.PlotableColored}*/
	...
	public static void main(String[] args) {
		...
		if(testType.equals("plotable")){
			...
			/*multi.addPlotable("route", new Traceroute(1,1,8,3));
			multi.addPlotable("route1", new Traceroute(4,1,5,15));
			multi.addPlotable("route2", new Traceroute(4,1,2,4));
			multi.addPlotable("route3", new Traceroute(2,4,5,15));
			multi.addPlotable("route4", new Traceroute(5,15,8,3));*/
			//multi.addPlotable("cercle4", new SemiCircu(1000,-1,1.));
			//multi.addPlotable("cercle2", new SemiCircu2(1000,-1,1.));
			ArrayList<Double> pa= new ArrayList<Double>();
			pa.add(1.) ; pa.add(-1.) ;pa.add(0.) ;pa.add(0.) ;pa.add(2.) ; //1-x+2x^4
			//pa.add(0.) ; pa.add(0.)  ;pa.add(1.); // x^2
			Polynome Pa= new Polynome(pa);
			Pa.toFonction(-1,1,500); // sur [-1,1]
			multi.addPlotable("Poly",Pa);
			...
			//multi.setColor("route2", Color.black);
			multi.setColor("Poly", Color.black);

			// on peut changer le style de trait.
			//multi.setStroke("Gauss", MultiPlotAncien.dashed);
			//multi.setStroke("Identité", MultiPlotAncien.thick);
			...
			//multi.setStroke("AutreConstante", MultiPlotAncien.thick);
			...
			multi.imposeRectangle(-1, -0.5, 1, 1);
		//multi.imposeRectangle(-1, -1, 1.2, 1); //Pour bien voir le cercle
