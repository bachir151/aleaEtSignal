...
//import aleat.tpD.CourbeDeGauss;
//import aleat.tpD.FonctionConstante;
//import aleat.tpD.FonctionId;
import aleat.tpJ.*;
...
public class MultiPlotGrille {
	...
	public void addPlotable(String name, Plotable plotable) {
...
//	}
	...
	public int[][] convertiToPix(Dimension dimPanneau, Plotable fonc) {
		...
		public void paintComponent(Graphics g1) {
			...
			for (PlotablePlus pp : sortedPlotablePlus) {
				...
				for (int i = 0; i < uneFoncPix[0].length; i++) {
					...
					/** on met des couleurs variantes, si le {@link MultiPlotGrille.MultiPlot.Plotable} est un {@link MultiPlotGrille.MultiPlot.PlotableColored}*/
					if (pp.normalizedColors != null) {
						if (pp.normalizedColors.get(i)==1.)
						g.setColor(Color.getHSBColor(/*pp.normalizedColors.get(i)20,1,255*/20,1,255)); //,1,1 bleue
						else g.setColor(Color.getHSBColor(/*pp.normalizedColors.get(i) 20,1,1*/20,1,1)); //,1,1
					...
					if (pp.joinType == JoinType.bullet || pp.joinType == JoinType.joinBullet) {
						...
						if (pp.normalisedBulletRadii != null) {
							/*if (pp.normalisedBulletRadii.get(i)==-1){//rajout
								pointSize = 0;}//rajout
							else{
								pointSize = 8;}//rajout*/
							pointSize = (int) Math.floor(pp.normalisedBulletRadii.get(i));/*vrai ligne*/
						}
	...
	public static void main(String[] args) {
		...
		//String testType="plotable";
		//String testType="plotableColors";
		...
		if(testType.equals("plotable")){
			...
			//multi.addPlotable("AutreConstante", new Ville(20));
			...
			//multi.setStroke("Gauss", MultiPlotGrille.dashed);
			//multi.setStroke("Identité", MultiPlotGrille.thick);
			...
			//multi.setJoinType("AutreConstante",JoinType.bullet,10); //RAJOUT
			//multi.setStroke("AutreConstante", MultiPlotGrille.thick);
			// on précise une hauteur, pour faire passer cette courbe au-dessus
			//multi.setZIndex("AutreConstante",50.);
			...
			//multi.addPlotable("Diffusion3", new Diffusion3());
			multi.plotNow();
		}
			...
			//multi.addPlotable("- identité pointé", new IdentityPointed(-1));
			//multi.setJoinType("identité pointé", JoinType.joinSquare);
	...
	private class PlotablePlus implements Comparable<PlotablePlus>{
		...
		public int defaultBulletRadius =6;
		public int maxBulletRadius =30;

		public PlotablePlus(String aName, Plotable aPlotable) {
			...
			/*	double maxRad=Double.MAX_VALUE;//-Double
				...
				double max =maximum(plotablePoint.radius());
				double min =minimum(plotablePoint.radius());
				boolean allEqual=false;
				if (max-min==0.) allEqual=true;

				normalisedBulletRadii =new ArrayList<Integer>();
				for (Double r:plotablePoint.radius()){
					//int norR;
					double norR;
					if (allEqual) norR= plotablePoint.radius().get(0);//norR=defaultBulletRadius;
					//else norR= (int) Math.floor( (r-min)/(max-min)*this.maxBulletRadius);
					if (r==1){norR=5;}
					else {norR=5;}
					if (norR<plotablePoint.radius().get(0)) norR=plotablePoint.radius().get(0);
					//if (norR<defaultBulletRadius) norR=defaultBulletRadius;
					this.normalisedBulletRadii.add((int)norR);
				...
				for (Double r:plotablePoint.radius()){
					int norR;
					if (allEqual) norR=defaultBulletRadius;
			...
			if (aPlotable instanceof PlotableColored) {
				...
				float max =(float) maximum(plotableColored.colors());
				float min =(float) minimum(plotableColored.colors());
				boolean allEqual=false;
				if (max-min==0.) allEqual=true;
